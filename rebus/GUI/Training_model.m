...
%%
    ...
    for i=1:length(segments)
        ...
        pause ;
        set(handles.textbox_click,'Visible','Off');
    end
    clc
    hold off;
    P1 = plot(time, Signal); set(P1, 'Color', [0.7 0.7 0.7]);    
...
% hObject    handle to button_data (see GCBO)
    ...
    flag=1;
    set(handles.button_data,'Visible','Off');
    col=get(handles.button_record,'BackgroundColor');
    while(get(handles.button_record,'Value')~=1 && flag==1)
    set(handles.button_record,'BackgroundColor','red');
    pause(1);
    set(handles.button_record,'BackgroundColor',col);
    pause(1);
    end
    set(handles.button_record,'BackgroundColor',col);
% eventdata  reserved - to be defined in a future version of MATLAB
...
% handles    structure with handles and user data (see GUIDATA)
 %   global word ;
    menu_label_Callback(hObject, eventdata, handles);
...
%%
    pop_string=get(handles.menu_word_list,'String');
    pop_value=get(handles.menu_word_list,'Value');
    word =strjoin(pop_string(pop_value));
    pop_string=get(handles.menu_voice_noise,'String');
    pop_value=get(handles.menu_voice_noise,'Value');
    label=strjoin(pop_string(pop_value));
    switch label 
        case 'Select nature'
            set(handles.menu_label,'String','No nature of segment choosen');
        case 'Noise'
...
%% call
textbox_other_label_Callback(hObject, eventdata, handles);
%% 
...
% hObject    handle to button_back_data (see GCBO)
    ...
    data(curent).gender=strjoin(fem_mal);
